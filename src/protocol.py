...
# ---------------------------------------------------------------------------
# Mensajes JSON con marco de longitud (protocolo del tracker)
# ---------------------------------------------------------------------------
async def read_json(reader: asyncio.StreamReader):
    """Lee un objeto JSON precedido por su longitud (4 bytes) + cuerpo.

    Usa readexactly, por lo que NO tiene limite de tamano. (Al contrario de
    readline, que en asyncio se limita a 64 KiB por defecto y desborda con
    manifiestos grandes; p. ej. el de un archivo de 4 GB con piezas de 2 MiB
    son ~2048 hashes SHA-256 ~= 140 KB y reventaba el buffer.)"""
    header = await reader.readexactly(4)
    (length,) = _HEADER.unpack(header)
    body = await reader.readexactly(length)
...
def encode_json(obj) -> bytes:
    """Serializa un objeto a JSON con marco de longitud (4 bytes) + cuerpo."""
