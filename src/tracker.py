...
class Tracker:
    ...
    async def _conn(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            req = await P.read_json(reader)        # marco de longitud: sin limite
        except (asyncio.IncompleteReadError, ConnectionError):
            writer.close()
            return
        try:
            resp = self.handle(req, client_host)
        except Exception as e:  # noqa: BLE001 (el tracker debe ser tolerante)
            resp = {"ok": False, "error": str(e)}
        try:
            writer.write(P.encode_json(resp))
            await writer.drain()
