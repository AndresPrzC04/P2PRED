...
STREAM_LIMIT = 8 * 1024 * 1024  # buffer de lectura por conexion de pares (8 MiB):
                                # holgado para piezas de 2 MiB, menos pausas de flujo
...
class Node:
    ...
    async def tracker_request(self, req: dict) -> dict:
        ...
        try:
            writer.write(P.encode_json(req))
            await writer.drain()
            return await P.read_json(reader)   # marco de longitud: sin limite de buffer
        finally:
            writer.close()
